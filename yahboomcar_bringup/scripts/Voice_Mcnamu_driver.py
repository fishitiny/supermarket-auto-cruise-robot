...
class yahboomcar_driver:
    def __init__(self):
        ...
        self.pub_flag = rospy.Publisher("/pub_flag", Int32, queue_size=1) #召唤标志位
        self.sub_command=rospy.Subscriber("/question",String,self.SubQuestionCallback,queue_size=1)	#语音命令
        self.sub_angle =rospy.Subscriber("/angle", Int32, self.SubAngleCallback,queue_size=1)#唤醒角度
        self.pub_SummonAngle =rospy.Publisher("/sunmon_angel", Int32,queue_size=1)#唤醒角度
        self.pub_goal = rospy.Publisher('move_base_simple/goal', PoseStamped, queue_size=1)
        self.dyn_server = Server(PIDparamConfig, self.dynamic_reconfigure_callback)
        self.car.create_receive_threading()
        self.summon_flag = Int32()
    ...
    def SubQuestionCallback(self,msg):
        print(msg.data)
        if msg.data == "前进":
            self.car.set_car_motion(0.3, 0, 0) 
            sleep(5)
            ...
            self.car.set_car_motion(0.3, 0, 0.5) 
            sleep(3)
            self.car.set_car_motion(0, 0, 0)
        elif msg.data == "右转":
            self.car.set_car_motion(0.3, 0, -0.5) 
            ...
            self.car.set_car_motion(0.0, 0, 0.3) 
            sleep(5)
            self.car.set_car_motion(0, 0, 0)
        elif msg.data == "右旋":
            self.car.set_car_motion(0, 0, -0.3) 
            sleep(5)
            self.car.set_car_motion(0, 0, 0)
        #summon
        elif msg.data == "过来":
            self.pub_SummonAngle.publish(self.angel)
            self.summon_flag.data = 1
            self.pub_flag.publish(self.summon_flag)
            
            #print(self.angel.data)
            print("---------------")
            
        #navigation
        elif msg.data == "去一号位":
            self.pose.pose.position.x = 1.62215459347
            self.pose.pose.position.y =  -0.0189398080111
            self.pose.pose.orientation.z = -0.00635431249085
            self.pose.pose.orientation.w = 0.999979811153
            self.pub_goal.publish(self.pose)
        elif msg.data == "去二号位":
            self.pose.pose.position.x = 1.8444968462
            self.pose.pose.position.y =  1.97917103767
            self.pose.pose.orientation.z = 0.728439719046
            self.pose.pose.orientation.w = 0.685109900466
            self.pub_goal.publish(self.pose)
        elif msg.data == "去三号位":
            self.pose.pose.position.x = -0.0620423592627
            self.pose.pose.position.y =  1.77574300766
            self.pose.pose.orientation.z = -0.652227962722
            self.pose.pose.orientation.w = 0.758022878707
            self.pub_goal.publish(self.pose)
            
    def SubAngleCallback(self,msg):
        #print("angle: ",msg.data)
        self.angel.data = msg.data
